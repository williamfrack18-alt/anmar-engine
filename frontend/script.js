document.addEventListener('DOMContentLoaded', () => {
    ...
    const briefStatePanel = document.getElementById('briefStatePanel');
    ...
    const chatChannelBadge = document.getElementById('chatChannelBadge');
    ...
    const chatTabHuman = document.getElementById('chatTabHuman');
    const humanIntroMsg = document.querySelector('#humanLog .msg-row.ai .ai-msg');
    ...
    function updateHumanIntro() {
        if (!humanIntroMsg) return;
        if (isMarketingChannel()) {
            humanIntroMsg.innerHTML = `
                > Soporte humano Anmar Marketing.<br>
                Escribe tu primer mensaje y te asignamos un estratega.<br><br>
                <span style="color:#38bdf8;">Ejemplo:</span> “Necesito una campaña para lanzar mi SaaS en 30 días.”
            `;
        } else {
            humanIntroMsg.innerHTML = `
                > Soporte humano Anmar.<br>
                Escribe tu primer mensaje y te asignamos un ingeniero.<br><br>
                <span style="color:#3b82f6;">Ejemplo:</span> “Necesito una web para mi estudio y un sistema de reservas.”
            `;
        }
    }
    function updateHumanTabLabel() {
        if (!chatTabHuman) return;
        chatTabHuman.textContent = isMarketingChannel() ? 'Soporte Marketing' : 'Soporte Humano';
    }
    function updateChatCopy(channel = activeChannel, tab = 'AI') {
        if (!chatInput) return;
        ...
        if (channel === 'marketing') {
            if (isHuman) {
                ...
                chatInput.placeholder = "Describe el objetivo de marketing, audiencia y oferta...";
                if (chatHelper) {
                    chatHelper.textContent = "La IA prepara el brief y los activos para redes sociales.";
                }
            }
            return;
        }
    ...
    document.addEventListener('chatTabSwitched', (e) => {
        updateChatCopy(activeChannel, e.detail || 'AI');
        updateHumanIntro();
        updateHumanTabLabel();
        resizeChatInput();
        updateSendState();
    ...
    function renderBriefState(meta = {}) {
        ...
        if (typeof meta.brief_score === 'number') {
            ...
            const totalFields = getRequiredFields().length || 1;
            latestBriefScore = Math.max(0, Math.min(100, Math.round(((totalFields - latestMissingFields.length) / totalFields) * 100)));
        }
            ...
            if (data.ready_for_handoff) {
                addSystemMessage(`
                    <div style="background:rgba(56,189,248,0.1); border:1px solid rgba(56,189,248,0.35); padding:16px; border-radius:12px; margin-top:12px;">
                        ...
                        <p style="color:#cbd5f5; font-size:0.85rem; margin:0 0 10px 0;">¿Quieres enviar esto al equipo humano para produccion y lanzamiento?</p>
    ...
    window.sendMarketingBrief = async function () {
        ...
        const payloadText = `${summary}\n\nActivos sugeridos:\n${assetLines || 'Pendientes de definicion.'}`;
        try {
    ...
    sendBtn.addEventListener('click', async () => {
        ...
        if (typeof isHumanChatActive !== 'undefined' && isHumanChatActive) {
            ...
            if (!window.__humanAssignedOnce) {
                ...
                const searchRow = addHumanSystemMessage(isMarketingChannel() ? 'Buscando estratega de marketing... 0s' : 'Buscando ingeniero disponible... 0s');
    ...
    function buildMemorySnapshot() {
        ...
        if (isMarketingChannel()) {
            ...
            snapshot.marketing_preview_assets = Array.isArray(currentMarketingAssets) ? currentMarketingAssets.slice(0, 6) : [];
        }
        return snapshot;
    }
    ...
    async function loadChatMemory() {
        ...
        try {
            ...
            setTimelineVisible(!isMarketingChannel() && (chatStage === 'construction_mode' || chatStage === 'building'));
            ...
            if (!rendered) {
                ...
                intro.innerHTML = `
                    <div class="ai-msg">
                        ...
                        ${memory.summary ? `Retomando contexto: ${escapeHtml(memory.summary)}` : (isMarketingChannel() ? 'No hay conversación previa de marketing. Describe el objetivo y empezamos.' : 'No hay conversación previa en este proyecto. Describe tu idea y empezamos.')}
                    </div>
                `;
    ...
    function renderMarketingPreview(assets = []) {
        ...
        list.forEach(asset => {
            const rawPlatform = String(asset.platform || '').toLowerCase();
            let platform = rawPlatform;
            if (rawPlatform.includes('instagram')) platform = 'instagram';
            if (rawPlatform.includes('tiktok')) platform = 'tiktok';
            if (rawPlatform.includes('youtube')) platform = 'youtube';
            const card = marketingPreviewContainer.querySelector(`.social-card[data-platform="${platform.charAt(0).toUpperCase() + platform.slice(1)}"]`)
                || marketingPreviewContainer.querySelector(`.social-card[data-platform="${asset.platform}"]`);
            if (!card) return;
    ...
    function updateWelcomeMessage() {
        if (!aiWelcomeMessage) return;
        if (isMarketingChannel()) {
            aiWelcomeMessage.innerHTML = `
                > Sistema Anmar Marketing listo.<br>
                > Conectado a Growth Core.<br><br>
                Describe el producto y el objetivo de la campana. Generare assets, copies y estrategia
                para TikTok, Instagram y YouTube.<br><br>
                <span style="color:#38bdf8;">¿Que vamos a lanzar hoy?</span>
            `;
        } else {
            aiWelcomeMessage.innerHTML = `
                > Sistema Anmar inicializado.<br>
                > Conexión establecida con el Núcleo Supra.<br><br>
                Estoy listo, arquitecto. Describe tu visión en lenguaje natural y yo generaré el plano,
                el código y la estrategia de financiación.<br><br>
    ...
    function setPreviewMode(mode) {
        ...
        if (mobileBtn) mobileBtn.style.display = isMarketing ? 'none' : 'inline-flex';
        if (desktopBtn) desktopBtn.style.display = isMarketing ? 'none' : 'inline-flex';
    }
    ...
    function setActiveChannel(channel) {
        ...
        if (chatChannelBadge) {
            if (isMarketingChannel()) {
                chatChannelBadge.textContent = 'MARKETING';
                chatChannelBadge.style.background = 'rgba(56,189,248,0.18)';
                chatChannelBadge.style.borderColor = 'rgba(56,189,248,0.35)';
                chatChannelBadge.style.color = '#e0f2fe';
            } else {
                chatChannelBadge.textContent = 'ENGINEERING';
                chatChannelBadge.style.background = 'rgba(16,185,129,0.18)';
                chatChannelBadge.style.borderColor = 'rgba(16,185,129,0.35)';
                chatChannelBadge.style.color = '#d1fae5';
            }
        }
        updateWelcomeMessage();
        updateChatCopy(activeChannel, isHumanChatActive ? 'Human' : 'AI');
        updateHumanIntro();
        updateHumanTabLabel();
        updatePaywallCopy();
        setPreviewMode(activeChannel);
        if (isMarketingChannel()) renderMarketingPreview(currentMarketingAssets);
        if (togglePreviewBtn) togglePreviewBtn.textContent = isMarketingChannel() ? 'Preview Marketing' : 'Preview';
        ...
        if (briefStatePanel) briefStatePanel.style.display = 'block';
        if (blueprintNowBtn) blueprintNowBtn.style.display = isMarketingChannel() ? 'none' : blueprintNowBtn.style.display;
        latestMissingFields = getRequiredFields().slice();
        renderBriefState({ missing_fields: latestMissingFields, memory_summary: '' });
        if (currentProjectName) {
            lastHumanChatCount = 0;
